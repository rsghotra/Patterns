...
T(N) = O(N) where N is number of nodes in the LL
S(N) = O(1)
*/
...
/*
Any number will be called a happy number if, after repeatedly replacing  with a number equal to the sum of the square of all of its digits, leads us to number ‘1’. All other (not-happy) numbers will never reach ‘1’. Instead, they will be stuck in a cycle of numbers which does not include ‘1’.
T(N) = O(logN); difficult to determine though
S(N) = O(1)
*/
...
void RearrangeLinkedList(ListNode* head) {
  ...
  //when list will have even elements; then last node's next will be pointing to itself;
  // 2->4->6->8->10->12 ==> 2->4->6->8<-10<-12
  //                                 |
  //                                null 
  //Hence, this check is mandatory
  if(leftPtr != 0) {
    leftPtr->next = 0; //this will not needed happen when list has odd elements then leftPtr will be null
  }
}
...
T(N) = O(N2); Note: did not prepare O(1) solution
S(N) = O(1)
*/
